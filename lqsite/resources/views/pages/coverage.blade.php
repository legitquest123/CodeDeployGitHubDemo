@extends('layout')
@section('content')

    <section class="section section-hero py-0 bg-gray">
      <div class="container">
        <div class="row row-grid justify-content-center">

          <div class="col-md-11 align-self-center text-center pt-2  mt-8 mb-4">

            <h1 class="mb-4 font-weight-bold">Legitquest Legal Database Coverage</h1>

            <div class="alert alert--narrow alert-secondary mb-5">
              <span class="alert-content">Whatever you need for your research, Legitquest has you covered.</span>
            </div> <!-- Alert -->

            <div class="coverage-list text-left">
              <div class="row">
                <div class="col-12 col-md-6 col-lg-4 offset-lg-2">
                  <ul _ngcontent-c20="" class="green-checks-list pl-0">
                    <li class="d-flex align-item-center">
                      <span class="check material-icons" cticon="check">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Supreme Court</strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">High Court</strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Tribunals </strong>
                    </li>
                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Central Bare Acts</strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">State Bare acts</strong>
                    </li>

                  </ul>

                </div>
                <div class="col-12 col-md-6 col-lg-4">
                  <ul class="green-checks-list pl-0">
                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Rules & Regulations</strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Notifications </strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Law commission reports </strong>
                    </li>

                    <li class="d-flex align-item-center">
                      <span class="check material-icons text-success" cticon="check ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"></path>
                        </svg>
                      </span>
                      <strong class="h4 font-weight-normal">Forms, Drafts Agreements, Deeds, Drafts Petitions etc. </strong>
                    </li>

                  </ul>

                </div>

              </div>

            </div>

          </div>

          <div class="col-12 text-center mt-auto mb-7">
            <div class="mb-2">
                <small> see our complete coverage list below.</small>
            </div>

            <a href="#sectionTeam" class="scroll-to icon icon--small icon--has-shadow d-inline-block rounded-circle background--white">
              <i class="fa fa-arrow-down"></i>
            </a>
          </div>

        </div>
      </div>
    </section> <!-- END Section Hero -->

    <section id="sectionTeam" class="section section-team gradient-light--upright">
      <div class="container">

        <div class="row row-grid justify-content-center">
          <div class="col-md-6 text-center">
            <h2 class="section__title mb-6 font-weight-bold">Complete Coverage</h2>
          </div>
        </div>

        <div class="complete-coverage-list list-unstyled">

            <div class="row mb-4">
              <div class="col-md-6">
                <div class="mb-4">
                  <strong class="h4 font-weight-bold">Supreme Court</strong>
                </div>
                <div class="row">
                  <div class="col-4">
                    <strong class="h4 font-weight-bold">High Courts</strong>
                  </div>
                  <div class="col-8">
                    <ul class="list-unstyled">
                    <li>Allahabad</li>
                    <li>Andhra Pradesh</li>
                    <li>Bombay </li>
                    <li>Calcutta </li>
                    <li>Chhattisgarh</li>
                    <li>Delhi</li>
                    <li>Guahati</li>
                    <li>Gujarat</li>
                    <li>Himachal Pradesh</li>
                    <li>Hyderabad</li>
                    <li>Jammu & Kashmir</li>
                    <li>Jharkhand</li>
                    <li>Karnataka</li>
                    <li>Kerela</li>
                    <li>Madhya Pradesh</li>
                    <li>Manipur</li>
                    <li>Meghalaya</li>
                    <li>Orissa</li>
                    <li>Patna</li>
                    <li>Punjab & Haryana</li>
                    <li>Rajasthan</li>
                    <li>Sikkim</li>
                    <li>Tamil Nadu</li>
                    <li>Tripura</li>
                    <li>Uttarakhand</li>
                    </ul>
                  </div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="row">
                  <div class="col-3">
                    <strong class="h4 font-weight-bold">Tribunals</strong>
                  </div>
                  <div class="col-9">
                    <ul class="list-unstyled">
                      <li>Authority for Advance Rulings</li>
                      <li>Armed Forces Tribunal</li>
                      <li>Appellate Tribunal For Electricity</li>
                      <li>Appellate Tribunal For Prevention of Money Laundering Act</li>
                      <li>Central Administrative Tribunal</li>
                      <li>Competition Commission of India</li>
                      <li>Customs Excise and Gold (Control) Appellate Tribunal</li>
                      <li>Central Electricity Regulatory Commission</li>
                      <li>Customs, Excise & Service Appellate Tribunal</li>
                      <li>Central Information Commission</li>
                      <li>Company Law Board</li>
                      <li>Competition Appellate Tribunal</li>
                      <li>Copyright Board</li>
                      <li>Central Sales Tax Appellate Authority</li>
                      <li>Debts Recovery Appellate Tribunal</li>
                      <li>Debts Recovery Tribunal</li>
                      <li>Intellectual Property Appellate Board</li>
                      <li>Income Tax Appellate Tribunal</li>
                      <li>Monopolies and Restrictive Trade Practices Commission</li>
                      <li>National Consumer Disputes Redressal Commission</li>
                      <li>National Company Law Appellate Tribunal</li>
                      <li>National Company Law Tribunal</li>
                      <li>National Green Tribunal</li>
                      <li>Privy Council</li>
                      <li>Real Estate Regulatory Authority Haryana</li>
                      <li>Registry of Trade Marks</li>
                      <li>Rajasthan Taxation Tribunal</li>
                      <li>Securities Appellate Tribunal</li>
                      <li>State Consumer Disputes Redressal Commission</li>
                      <li>Securities Exchange Board of India</li>
                      <li>Sales Tax Tribunal</li>
                      <li>Telecom Disputes Settlement and Appellate Tribunal</li>
                      <li>Tamil Nadu Taxation Special Tribunal</li>
                      <li>Union Territory Consumer Disputes Redressal Commission</li>
                      <li>West Bengal Taxation Tribunal</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>

            <hr class="mt-5 mb-5">

            <div class="row">
              <div class="col-md-4">
                <div class="mb-5">
                  <strong class="h4 font-weight-bold">Notifications and circulars of Government of India and their departments.</strong>
                </div>
              </div>
              <div class="col-md-4">
                <div class="mb-5">
                  <strong class="h4 font-weight-bold">Central Bare acts</strong>
                </div>
              </div>
              <div class="col-md-4">
                <div class="mb-5">
                  <strong class="h4 font-weight-bold">State Bare Acts</strong>
                </div>
              </div>
              <div class="col-md-4">
                <div class="mb-5">
                  <strong class="h4 font-weight-bold">Rules and Regulations </strong>
                </div>
              </div>
              <div class="col-md-4">
                <div class="mb-5">
                  <strong class="h4 font-weight-bold">Law Commission Reports</strong>
                </div>
              </div>
              <div class="col-md-4">
                <div class="row">
                  <div class="col-7">
                    <strong class="h4 font-weight-bold">Supreme Court, High court, Tribunals and Subject wise legal drafts</strong>
                  </div>
                  <div class="col-5">
                    <ul class="list-unstyled">
                      <li>Agreements</li>
                      <li>Affidavits</li>
                      <li>Applications</li>
                      <li>Contracts</li>
                      <li>Petitions </li>
                      <li>Complaints </li>
                      <li>Forms </li>
                      <li>Legal Notices</li>
                      <li>Wills</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>

            <div class="row">

            </div>

        </div>
      </div>
    </section> <!-- END SECTION TEAM -->




    <section class="section section-subscribe gradient-light--upright">
      <figure class="figure pattern-svg">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1306px" height="225px">
          <path fill-rule="evenodd" fill="rgb(35, 23, 123)" d="M-0.011,9.318 L8.682,6.988 L11.011,15.682 L2.318,18.011 L-0.011,9.318 Z"></path>
          <path fill-rule="evenodd" fill="rgb(35, 23, 123)" d="M607.694,164.319 L612.680,161.693 L615.306,166.680 L610.320,169.305 L607.694,164.319 Z"></path>
          <path fill-rule="evenodd" fill="rgb(93, 203, 250)" d="M667.343,205.646 L679.152,213.179 L671.620,224.988 L659.810,217.456 L667.343,205.646 Z"></path>
          <path fill-rule="evenodd" fill="rgb(93, 203, 250)" d="M915.839,-0.008 L924.997,5.833 L919.156,14.991 L909.998,9.150 L915.839,-0.008 Z"></path>
          <path fill-rule="evenodd" fill="rgb(35, 23, 123)"
            d="M989.000,93.000 C993.418,93.000 997.000,96.582 997.000,101.000 C997.000,105.418 993.418,109.000 989.000,109.000 C984.582,109.000 981.000,105.418 981.000,101.000 C981.000,96.582 984.582,93.000 989.000,93.000 Z"></path>
          <path fill-rule="evenodd" fill="rgb(35, 23, 123)"
            d="M1165.000,8.000 C1167.761,8.000 1170.000,10.238 1170.000,13.000 C1170.000,15.761 1167.761,18.000 1165.000,18.000 C1162.239,18.000 1160.000,15.761 1160.000,13.000 C1160.000,10.238 1162.239,8.000 1165.000,8.000 Z"></path>
          <path fill-rule="evenodd" fill="rgb(93, 203, 250)"
            d="M1303.500,118.000 C1304.881,118.000 1306.000,119.119 1306.000,120.500 C1306.000,121.880 1304.881,123.000 1303.500,123.000 C1302.119,123.000 1301.000,121.880 1301.000,120.500 C1301.000,119.119 1302.119,118.000 1303.500,118.000 Z">
          </path>
          <path fill-rule="evenodd" fill="rgb(93, 203, 250)"
            d="M61.000,112.000 C62.657,112.000 64.000,113.343 64.000,115.000 C64.000,116.657 62.657,118.000 61.000,118.000 C59.343,118.000 58.000,116.657 58.000,115.000 C58.000,113.343 59.343,112.000 61.000,112.000 Z"></path>
          <path fill-rule="evenodd" fill="rgb(74, 92, 246)"
            d="M265.500,-0.000 C271.299,-0.000 276.000,4.701 276.000,10.500 C276.000,16.299 271.299,21.000 265.500,21.000 C259.701,21.000 255.000,16.299 255.000,10.500 C255.000,4.701 259.701,-0.000 265.500,-0.000 Z"></path>
          <path fill-rule="evenodd" fill="rgb(35, 23, 123)"
            d="M185.500,131.000 C187.985,131.000 190.000,133.015 190.000,135.500 C190.000,137.985 187.985,140.000 185.500,140.000 C183.015,140.000 181.000,137.985 181.000,135.500 C181.000,133.015 183.015,131.000 185.500,131.000 Z"></path>
        </svg>
      </figure> <!-- Pattern SVG Background -->

      <div class="container">
        <div class="row row-grid justify-content-center">

          <div class="col-md-8 col-lg-7 col-xl-6 text-center">
            <h2 class="section__title mb-4">Start Using Legitquest Now</h2>

            <p>Try Legitquest and see the difference</p>

            <form class="subscribe-form form-inline mt-5">
              <div class="input-group mr-sm-3 mb-2">
                <input type="email" class="form-control" id="email" placeholder="Email address">
              </div>
              <button type="submit" class="btn btn-primary btn--has-shadow mb-2">Get Started Now</button>
            </form>
          </div>

        </div>
      </div>
    </section> <!-- END Section Subscribe -->

@endsection